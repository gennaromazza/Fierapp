...
export function DynamicChatGuide() {
  ...
  // Usa gli items da useCartWithRules invece di stato locale
  const items = cart.getAllItemsWithAvailability().map(item => ({
    id: item.id,
    title: item.title,
    price: item.price,
    ...
    ruleSettings: item.ruleSettings,
    mutuallyExclusiveWith: item.mutuallyExclusiveWith,
    createdAt: item.createdAt || new Date(),
    updatedAt: item.updatedAt || new Date(),
  }));
  
  // Debug per vedere se i dati arrivano
  useEffect(() => {
    console.log('🔍 DynamicChatGuide: Items from cart:', items.length, items.map(i => ({ id: i.id, title: i.title, category: i.category })));
    console.log('🔍 DynamicChatGuide: Cart loading:', cart.rulesLoading);
  }, [items.length, cart.rulesLoading]);
  ...
  useEffect(() => {
    // DISCOUNTS: mantieni onSnapshot per discounts
    ...
    setTimeout(() => {
      ...
      if (services.length > 0) {
        ...
        addMessage({
          ...
          type: 'assistant',
          avatar: 'thoughtful',
          text: "⚠️ Nessun servizio disponibile al momento. Ricarica la pagina per riprovare.",
        });
      }
    }, 2000);
  };
